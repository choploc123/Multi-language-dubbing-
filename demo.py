...
def split_audio_video(input_video):
    ...
    audio_dir = "0_Extracted Audio"
    video_dir = "0_Video without Audio"
    clip_dir = "0_10s Audio Clip"
...
def run_wav2lip(video, audio):
    ...
    video_path = video  # Đã là đường dẫn
    audio_path = audio  # Đã là đường dẫn
...
lipsync_interface = gr.Interface(
    fn=run_wav2lip,
    inputs=[
        gr.Video(label="🎥 Select Video"),  # Đã bỏ type
        gr.Audio(label="🎵 Select Audio", type="filepath")  # Audio vẫn dùng filepath để dễ xử lý
    ],
    outputs=gr.Video(label="🔁 Processed Video", show_download_button=True),
    title="LipSync with Wav2Lip",
    description="🗣️ Upload a video and audio file. You can preview both inputs before syncing lips."
)
